...
namespace file_sorter {
	namespace platform {
		...
		control_t new_control(window_t parent, control_data* data) {
			...
			auto window = (windows::windows_window*)parent;
			switch (data->type) {
			case control_type::button:
			{
				...
				ctrl->window = CreateWindowA("Button", btn_data->text.c_str(), WS_VISIBLE | WS_CHILDWINDOW, btn_data->x, btn_data->y, btn_data->width, btn_data->height, window->window, (HMENU)index, NULL, NULL);
			}
				break;
			case control_type::label:
			{
				label_data* lbl_data = (label_data*)data;
				ctrl->window = CreateWindowA("Static", lbl_data->text.c_str(), WS_VISIBLE | WS_CHILDWINDOW, lbl_data->x, lbl_data->y, lbl_data->width, lbl_data->height, window->window, NULL, NULL, NULL);
			}
				break;
			}
			return ctrl;
